...
function updateDropdowns(optiontext, appendedText) {
    ...
    $('#confidencepoints2 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints3 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints4 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints5 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints6 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints7 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints8 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints9 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints10 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints11 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints12 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints13 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints14 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints15 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints16 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints17 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints18 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints19 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints20 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints21 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints22 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints23 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints24 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
    $('#confidencepoints25 option').filter(function() {
      return $(this).val().trim() == optiontext;
    }).text(optiontext + appendedText);
}
...
function profileScores(selectionDisplay, correctSelections, totalScore, incorrectSelections, week) {
    ...
    currentTotalScore.innerHTML = "Week " + week.toString() + " Score: " + totalScore;
...
function populateWeeklyScores(weeklyScoresDict, correctSelections, incorrectSelections, week){
    ...
    for (let i = 0; i < Object.keys(orderedScores).length; i++) {
        ...
        for(let j = 0; j < 2; j++) {
            ...
            for(let k = 0; k < Object.keys(orderedScores[i][1]['selections']).length; k++) {
                const td = tr.insertCell();
                if (j == 0) {
                    var cell = document.createElement("img");
                    cell.src = teamLogos[Object.keys(orderedScores[i][1]['selections'])[k]];
                    ...
                    if(correctSelections.toString().split(',').includes(Object.keys(orderedScores[i][1]['selections'])[k])){
                        textColor = "green"
                    } else if(incorrectSelections.toString().split(',').includes(Object.keys(orderedScores[i][1]['selections'])[k])){
                        textColor = "red"
                    } else {
                        textColor = "black"
                    }
                    cell.innerHTML = orderedScores[i][1]['selections'][Object.keys(orderedScores[i][1]['selections'])[k]];
                    td.style.border = '1px solid black';
